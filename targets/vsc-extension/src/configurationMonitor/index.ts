...
export class ConfigurationMonitor {
            ...
            async (content, config, serverKey, servers) => {
                ...
                for (const [serverName, serverConfig] of Object.entries(servers)) {
                    ...
                    try {
                        ...
                        // Add proper indentation to match the original structure
                        const indentedRawConfig = this.addProperIndentation(
                            rawConfig,
                            serverNode.offset,
                            modifiedContent
                        );

                        modifiedContent = before + indentedRawConfig + after;
                        hasChanges = true;
                    } catch (error) {
    ...
    /**
     * Add proper indentation to raw JSONC content to match original file structure
     */
    private addProperIndentation(
        rawJsonc: string,
        offset: number,
        originalContent: string
    ): string {
        // Find the indentation level by looking at the line containing the offset
        const beforeOffset = originalContent.substring(0, offset);
        const lastNewlineIndex = beforeOffset.lastIndexOf("\n");
        const currentLine = originalContent.substring(lastNewlineIndex + 1, offset);
        const indentation = currentLine.match(/^(\s*)/)?.[1] || "";

        // Split the raw JSONC into lines and add proper indentation
        const lines = rawJsonc.split("\n");
        return lines
            .map((line, index) => {
                if (index === 0) {
                    return line; // First line keeps original indentation
                }
                return indentation + line; // Subsequent lines get the current indentation
            })
            .join("\n");
    }
